...
answeer:- app.listen() starts the server and makes it listen for incoming client requests on a specified port number.


✔️ Why router is not a server?

Router cannot listen on a port; it only handles grouped routes when attached to an Express app.

Why use routers?
To keep code modular, readable, and scalable.

✔️ What happens first: server or router?
Server is created first, routers are attached, then server starts listening.
...
👉 If a field stores another document’s _id → use ObjectId
👉 If a field stores normal text → use String