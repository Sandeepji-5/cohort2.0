...
const handleUpdate = async (e) => {
  ...
  await axios.patch(
    `http://localhost:3000/api/notes/${eyed}`, async(req, res)=>{
    const id = req.params.id
    const title2 = form.elements.title.value.trim();
    const description2 = form.elements.description.value.trim();
    await noteModel.findByIdAndUpdate(eyed, {title2, description2});
    res.status(200).json({
